...
const boardMap = new Map(communityBoards.map((board) => [board.slug, board] as const))
</script>
          ...
          <article v-for="post in feed" :key="post.id" class="feed-card">
            <div class="feed-card__meta">
              <span class="feed-card__board">
                {{ boardMap.get(post.boardSlug)?.emoji }}
                {{ boardMap.get(post.boardSlug)?.title ?? '커뮤니티' }}
              </span>
            </div>
            <h4>{{ post.title }}</h4>
            <p>{{ post.summary }}</p>
      ...
      <aside class="sidebar">
        <section class="board-stack">
          ...
          <div class="board-grid">
            <article v-for="board in boardShowcase" :key="board.slug">
        ...
        <section class="tag-panel">
          ...
          <ul>
            <li v-for="tag in trendingCommunityTags" :key="tag.label">
              <div>
