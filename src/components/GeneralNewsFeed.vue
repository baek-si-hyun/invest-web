...
const resolveRegionLabel = (regionId: RegionId) => (regionId === 'all' ? '전체' : regionId)
...
const feedHeadline = computed(() =>
  activeSource.value ? `${activeSource.value.label} 헤드라인` : '글로벌 뉴스 브리핑'
)

const feedDescription = computed(() =>
  activeSource.value
    ? `${activeSource.value.label}의 최신 헤드라인을 한눈에 살펴보세요.`
    : '주요 글로벌 언론사의 최신 헤드라인을 큐레이션했습니다.'
)

const feedCountLabel = computed(() =>
  activeSource.value
    ? `${activeSource.value.label} 기사 ${filteredNews.value.length}건`
    : `${resolveRegionLabel(activeRegion.value)} 기사 ${filteredNews.value.length}건`
)
              ...
              <header class="card-header">
                <span class="badge" :style="{ background: news.accent }">
                  <img
                    ...
                    loading="lazy"
                  />
                  <span v-else class="badge-icon" aria-hidden="true">{{ news.icon ?? '📰' }}</span>
